...
<div class="wrap">
    <div class="bread_box">
        ...
        <span> &gt;</span>
        <a href="${pageContext.request.contextPath}/routeController/routeInfo?currentPage=1&count=10&cid=${routeDetails.category.cid}">${routeDetails.category.cname}</a><span> &gt;</span>
        <a href="#">${routeDetails.rname}</a>
    </div>
    <div class="prosum_box">
        <dl class="prosum_left">
            <dt>
                <img alt="" class="big_img" src="${pageContext.request.contextPath}/${routeDetails.rimage}">
            </dt>
            <dd>
                <a class="up_img up_img_disable"></a>
                <c:forEach items="${routeDetails.routeImgList}" var="routeImg">
        ...
        <div class="prosum_right">
            <p class="pros_title">${routeDetails.routeIntroduce}</p>
            <p class="hot">1-2月出发，网付立享￥1099/2人起！爆款位置有限，抢完即止！</p>
            <div class="pros_other">
                <p>经营商家  ：${routeDetails.seller.sname}</p>
                <p>咨询电话 : ${routeDetails.seller.consphone}</p>
                <p>地址 ： ${routeDetails.seller.address}</p>
            </div>
            <div class="pros_price">
                <p class="price">
                    <strong>￥${routeDetails.price}</strong>
                </p>
                <div class="p_number">
                    ...
                    <span class="collect">
                         <a class="btn" href="javascript:void(0)" id="addCart"><i class="glyphicon glyphicon-heart-empty"></i>加入购物车</a>
                    </span>
                </div>
                <%--加入购物车--%>
                <script>
                    $("#addCart").click(function(){
                        //获取rid
                        let rid=${routeDetails.rid};
                        //获取数量
                        let num=$("#qty_item_1").val();
                        $.ajax({
                            url:"${pageContext.request.contextPath}/routeController/addCommodityToCart",
                            data:"rid="+rid+"&num="+num,
                            type:"get",
                            dataType:"json",
                            success:function (resp) {
                                if (!resp.success) {
                                    ...
                                    let count=resp.object.num;
                                    let subprice=resp.object.subCost;
                                    location.href="${pageContext.request.contextPath}/routeController/cartSuccess?rid=${routeDetails.rid}&num="+count+"&subCost="+subprice;
                                }
                            },
                            error:function () {
                               alert("服务器繁忙...") ;
...
<style type="text/css">
    *{margin:0;padding:0;}
    ...
    .p_number .add_chose a{float:left;margin:5px 0 0 0;display:block;width:15px;height:15px;line-height:99em;overflow:hidden;background:url(${pageContext.request.contextPath}/images/reduce-add.gif) no-repeat;}
